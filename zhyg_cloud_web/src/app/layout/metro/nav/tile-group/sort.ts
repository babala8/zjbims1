...
export class LayoutProcessor {
    ...
    private _calculateGroupWidth(array: Array<IRectangle>) {
        let area: number = 0, maxTileWidth: number = 0;
        ...
        // console.log('area:' + area + ',maxTile:' + maxTileWidth);
        if (area > 8) {
            area = 8;
            maxTileWidth = 4;
        }
    ...
    doHandleByLine(base: Offset) {
        ...
        let maxTileHeight = this.getMaxTileHeight(),
        ...
        const problem = this.gridMap[`${this.groupWidth}*${maxTileHeight}`];
        const strategy = this._handle(problem(base.offsetX, base.offsetY)());
        if (strategy) {
    ...
    createFreeSpaceList(offset: Offset) {
        const height = this.getMaxTileHeight();
        this.fillSpace([{
    ...
    fillSpace(freeSpaceList: Array<Space>, nextOffset: Offset) {
        ...
        let minArea = 100, minSpaceIndex, minSpace;
        // 1、找到最小的空闲空间
        freeSpaceList.forEach((item: Space, index: number) => {
            const product = item.width * item.height;
            if (product < minArea) {
                minArea = product;
                minSpaceIndex = index;
            }
        ...
        let totalTiles = [];
        const keyArray = ['tile-large', 'tile-wide', 'tile', 'tile-small'];
    ...
    calculatePosition(strategy: Array<StrategyItem>) {
        strategy.forEach((item: StrategyItem) => {
            const tileType = item['tile-type'];
            switch (tileType) {
                case 'tile-small':
                    const tileSmallArray = this.map['tile-small'];
                    const tempArray = tileSmallArray.splice(0, 4);
