...
export class PasswordComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.validateForm = this.fb.group({
            ...
            oldPassword: [null, [Validators.required, Validators.pattern('^(?!([a-zA-Z]+|\\d+)$)[a-zA-Z\\d]{8,16}')]],
            newPassword: [null, [Validators.required, Validators.pattern('^(?!([a-zA-Z]+|\\d+)$)[a-zA-Z\\d]{8,16}')]],
    ...
    _submitForm() {
        ...
        const newPassword = this.validateForm.controls.newPassword.value.toString();
        const repeatNewPassword = this.validateForm.controls.repeatNewPassword.value.toString();
        if (newPassword !== repeatNewPassword) {
            this.nzMessage.info('两次密码输入不一致');
            return;
        }
