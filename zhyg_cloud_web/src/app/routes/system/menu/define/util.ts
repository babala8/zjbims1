 ...
 * 返回：
 *
 * 注: 该算法很消耗资源，如有必要可以考虑数组去重的简单算法，将所有的id放到一个Array中
 *
 */
...
/**
 * 1.对类型为0的节点，增加group属性
 * 2.移除children为空数组
 *
 */

export function removeEmptyChildren(nodes: Array<any>) {
    for (let i = 0; i < nodes.length; i++) {
        if (nodes[i].type === 0) {
            nodes[i].group = true;
        } else {
            if (nodes[i].group) delete nodes[i].group;
        }
        if (nodes[i].children) {
            if (nodes[i].children.length === 0) {
                delete nodes[i].children;
            } else {
                removeEmptyChildren(nodes[i].children);
            }
        }
    }
}
