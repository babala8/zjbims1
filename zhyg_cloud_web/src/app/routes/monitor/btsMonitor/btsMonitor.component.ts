...
@Component({
    templateUrl: './btsMonitor.html',
})
export class BtsMonitorComponent implements OnInit, OnDestroy {
    ...
    cardstatus = [  // 监控状态-卡箱状态
        ...
        {
            label: '未知', name: 'card4', value: 1, checked: true, img: 'assets/images/operate/cardunknown.png'
        }
    ];
    ...
    awayFlagInfo = [{no: '0', name: '全部'}, {no: '1', name: '在行'}, {no: '2', name: '离行'}]; // 样式
    PageSizeSelectorValues = [48, 72, 96, 144, 192];

    constructor(
        private fb: FormBuilder,
        private devMonitorService: DevMonitorService,
        private crdMonitorService: BtsMonitorService,
    ...
    ngOnInit() {
        ...
        this.monitorData.monitorType = '6'; // 监控类型为回單机
        ...
        this.reqdevlistdata.monitorType = '6';
    ...
    ngOnDestroy(): void {
        console.log('destroy', this.timeref);
        window.clearInterval(this.timeref);
    }
    ...
    onMatrixEvent(matrixEventdata) {
        const modal = this.modal.open({
            title: '设备详细信息',
            content: BtsMonitorDetailComponent,
            width: '80%',
            componentParams: {
    ...
    useInterval() {
        ...
        if (window.btsTimers != null) {
          ...
          this.timeref = setInterval(() => {
           const that = this;
              Object.defineProperty(window, 'btsTimers', {
