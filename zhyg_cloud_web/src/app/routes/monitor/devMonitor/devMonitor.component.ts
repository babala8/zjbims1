...
@Component({
    templateUrl: './devMonitor.html',
})
export class DevMonitorComponent implements OnInit, OnDestroy {
    ...
    runstatus = [
        ...
        {
            label: '停止服务', name: 'run2', value: 1, checked: true, img: 'assets/images/operate/stopservice.png'
        },
        {
            label: '部分服务', name: 'run3', value: 1, checked: true, img: 'assets/images/operate/Partynormal.png'
        },
        {
    ...
    PageSizeSelectorValues = [48, 72, 96, 144, 192];
    ...
    ngOnInit() {
        ...
        this.monitorData.monitorType = '1';
        ...
        this.reqdevlistdata.monitorType = '1';
    ...
    getDevListCom(): void {
        ...
        this.devMonitorService.getDevList(this.reqdevlistdata)
            .subscribe(_data => {
                ...
                console.log(_data.page.totalRow);
    ...
    onMatrixEvent(matrixEventdata) {
        const modal = this.modal.open({
            title: '设备详细信息',
            content: DevMonitorDetailComponent,
            width: '80%',
            componentParams: {
    ...
    onChange(): void {
        ...
        if ( isNaN(this.inputValue) || this.inputValue < 5 || this.inputValue > 99 ) {
            this.message.error('请输入，大于4 小于100的数字');
            this.inputValue = 20;
        }else {
       ...
       /* if (this.inputValue < 5 || this.inputValue > 99) {
        ...
        // const reg = /^[1-9][0-9]?$/;
        // if (!value.match(reg)) {
        //     this.message.error('输入了大于5或小于100的数字');
        //     this.validateForm.controls.monitorInterval.setValue(this.Ex_monitorInterval);
        //     return;
        // }
        // const interval = parseInt(this.inputValue, 10);
        ...
        this.devMonitorService.updateUserMonitorSelect(params).subscribe(_data => {
            ...
            this.message.info('刷新频率改变为：' + this.monitorInterval + ' 秒/次');
        }, (error) => {
            if (error instanceof HttpResponse) {
                this.message.error(error.body.retMsg);
            }
    ...
    useInterval() {
        ...
        this.timeref = setInterval(() => {
            const that = this;
            Object.defineProperty(window, 'devTimers', {
                value : {user: this.session.getUserSession().count, interval: that.timeref } ,
                writable : true,
            });
            this.getDevListCom();
            console.log('interval', this.timeref);
        }, this.monitorInterval * 1000);
