...
@Injectable()
export class TransReportService {
    ...
    getorgTypeInfo(params): Observable<any> {
        return this.http.get<any>(this.devsUrl + '/SysOrgWorkTime/orgTypeName', {
            params: params
        });
    }

    branchTrans(params) {
        window.location.href = this.devsUrlReports + '/trans/branchTrans?' + this.parseParams(params);
    }
    doubtfulTrans(params) {
        window.location.href = this.devsUrlReports + '/trans/doubtfulTrans?' + this.parseParams(params);
    }
    devTransInfo(params) {
        window.location.href = this.devsUrlReports + '/trans/devTransInfo?' + this.parseParams(params);
    }
    reconciliation(params) {
        window.location.href = this.devsUrlReports + '/trans/reconciliation?' + this.parseParams(params);
    }
    earning(params) {
        window.location.href = this.devsUrlReports + '/trans/gains?' + this.parseParams(params);
    }
    atmCash(params) {
        window.location.href = this.devsUrlReports + '/trans/ATMCash?' + this.parseParams(params);
    }
